import { Node, Tree, derivedTriangle, resolveArrows } from "./tree.js";
/**
...
function tokenize(input) {
    ...
    while (i < input.length) {
        ...
        if (isSpace(ch)) {
            ...
            let j = i;
            while (j < input.length && !isSpace(input[j]) && !DELIMITERS.has(input[j]))
                j++;
 ...
 */
function readAnnotations(tokens, i, node, allowArrow) {
    let pos = i;
    while (pos < tokens.length) {
        ...
        if (marker.type !== "sub" && marker.type !== "sup")
            break;
