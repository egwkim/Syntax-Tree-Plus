 ...
 */
export function addChildAt(node, index, label = "", isWord = false) {
...
export function addChild(node, label = "", isWord = false) {
 ...
 */
export function addSiblingAfter(node, label = "", isWord = node.isWord) {
    if (!node.parent)
        return null;
...
export function addSiblingBefore(node, label = "", isWord = node.isWord) {
    if (!node.parent)
        return null;
    const sibling = new Node(label);
    sibling.isWord = isWord;
...
export function addSibling(node, label = "") {
 ...
 */
export function reparent(node, newParent, index) {
    ...
    let idx = index;
    if (sameParent && originalIndex !== -1 && originalIndex < index)
        idx -= 1;
...
export function deleteNode(tree, node) {
    if (!node.parent)
        return node; // never delete the root
...
export function wrapNode(tree, node, label = "X") {
...
export function moveSibling(node, dir) {
    if (!node.parent)
        return false;
 ...
 */
export function xbarTemplate(tree, node) {
 ...
 */
export function cpTpTemplate(tree, node) {
    node.updateLabel("CP");
    [...node.children].forEach((c) => node.removeChild(c));
 ...
 * ("and" / "or" / ...).
 */
export function coordinationTemplate(tree, node) {
...
export function toggleTriangle(node) {
    if (node.isWord)
 ...
 */
export function toggleWordNode(node) {
    if (!node.isLeaf)
        return false;
 ...
 */
export function applyAutoSubscripts(tree, enabled) {
    tree.root.walk((n) => {
        n.autoSubscript = "";
    });
    if (!enabled)
    ...
    tree.root.walk((n) => {
        if (n.isWord)
            return; // words aren't numbered
        if (n.subscript)
            return; // respect an explicit subscript
    ...
    groups.forEach((nodes) => {
        if (nodes.length < 2)
            return; // only repeated labels get numbered
        nodes.forEach((n, i) => {
 ...
 * nowhere.
 */
export function setArrow(source, target, ends) {
    if (source === target || !source.isWord || !target.isWord)
        return false;
    source.arrow = { target, rawColumn: 0, ends: { ...ends } };
    return true;
}
/** Remove the arrow leaving `node`, if any. Returns whether one was removed. */
export function clearArrow(node) {
    if (!node.arrow)
        return false;
    node.arrow = null;
    return true;
 ...
 */
export function nextArrowEnds(current) {
    if (!current)
        return ARROW_CYCLE[0];
    const i = ARROW_CYCLE.findIndex((e) => e.to === current.to && e.from === current.from);
