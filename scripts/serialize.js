import { wordColumnIndex } from "./tree.js";
/**
...
function scriptSuffix(node) {
    let s = "";
    if (node.superscript)
        s += "^" + quoteIfNeeded(node.superscript);
    if (node.subscript)
        s += "_" + quoteIfNeeded(node.subscript);
    return s;
}
/** Rebuild a raw label token, e.g. base="N", sub="1", sup="0" -> "N^0_1". */
function labelToken(node) {
 ...
 */
function arrowSuffix(node, columns) {
 ...
 */
function terminalToken(node, standalone, columns) {
    const label = node.label;
    const quote = standalone || TERMINAL_NEEDS_QUOTING.test(label) || ARROW_TOKEN.test(label);
 ...
 * quotes on `the`.
 */
function hasWordNeighbour(children, i) {
    return ((i > 0 && children[i - 1].isWord) ||
 ...
 */
export function serializeNode(node, columns) {
    const cols = columns ?? wordColumnIndex(node.tree ? [node.tree] : []);
    if (node.isWord)
        return terminalToken(node, false, cols);
    const parts = node.children.map((child, i) => child.isWord
...
export function serialize(tree, columns) {
 ...
 * {@link parseAll}.
 */
export function serializeAll(trees, separator = "\n") {
 ...
 */
export function serializePretty(tree, indent = "  ", columns = wordColumnIndex([tree])) {
    const build = (node, depth, standalone) => {
 ...
 */
export function serializePrettyAll(trees, indent = "  ", separator = "\n\n") {
    const columns = wordColumnIndex(trees);
