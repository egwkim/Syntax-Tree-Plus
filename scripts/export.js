...
export function svgString(trees) {
 ...
 */
function renderPNGBlob(trees, scale = 1, transparent = false) {
 ...
 */
export function uniqueFilenames(names, ext) {
    const used = new Map();
...
/** Build one tab's PNG. */
export async function pngFile(trees, filename, scale = 1, transparent = false) {
...
export function svgFile(trees, filename) {
...
export function latexFile(trees, filename) {
    return {
...
export function clipboardTextSupported() {
 ...
 */
export async function copyPNG(trees, scale = 1, transparent = false) {
 ...
 */
export async function copySVGImage(trees) {
    requireClipboardImage();
    const { xml } = svgString(trees);
 ...
 */
export async function copySVGMarkup(perTab) {
...
function scriptSuffix(node) {
    ...
    if (!sub && !node.superscript)
        return "";
    let s = "$";
    if (node.superscript)
        s += "^{" + texEscape(node.superscript) + "}";
    if (sub)
        s += "_{" + texEscape(sub) + "}";
    return s + "$";
}
/** Generate LaTeX code for the `forest` package (one tree). */
function treeToForest(tree) {
    ...
    const build = (node, depth) => {
        ...
        if (node.isLeaf) {
            ...
            const content = (node.isWord && !node.triangle ? `\\textit{${base}}` : base) +
                scriptSuffix(node);
