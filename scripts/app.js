...
import { settings, applyThemeColors, SETTING_LIMITS, clampSetting, resetDisplaySettings, } from "./settings.js";
import { parseAll, parseLabel } from "./parser.js";
import { serializeAll, serializePrettyAll } from "./serialize.js";
import { History } from "./history.js";
import { Workspace, parseTabSelection } from "./tabs.js";
import { setupCompactToolbar } from "./toolbar.js";
import { COMMANDS, FIXED_KEYS, bindingFor, commandForKey, canonicalFromEvent, displayKey, applyOverrides, overrides as keymapOverrides, resetBindings, rebind, } from "./keymap.js";
import { saveTheme, loadTheme, savePrefs, loadPrefs, saveWorkspace, loadWorkspace, loadDoc, fragmentDoc, clearFragment, shareURL, saveKeymap, loadKeymap, } from "./persist.js";
import { pngFile, svgFile, latexFile, downloadFiles, uniqueFilenames, copyPNG, copySVGImage, copySVGMarkup, copyLaTeX, clipboardImageSupported, clipboardTextSupported, } from "./export.js";
import { matchBrackets, bracketPairAtCaret, matchingOpen, buildHighlightHTML, diffRange, adjustIndex, } from "./brackets.js";
import { cloneNodeSubtree, addChildAt, addSiblingBefore, addSiblingAfter, deleteNode, wrapNode, moveSibling, xbarTemplate, cpTpTemplate, coordinationTemplate, toggleTriangle, toggleWordNode, isDescendant, reparent, setArrow, clearArrow, nextArrowEnds, } from "./edit.js";
const DEFAULT_DOC = "[S [NP [D The][N cat]] [VP [V sat] [PP [P on] [NP [D the][N mat]]]]]";
export function startApp() {
    ...
    const hSpacingInput = document.getElementById("setting-h-spacing");
    const vSpacingInput = document.getElementById("setting-v-spacing");
    const edgeStyleSelect = document.getElementById("setting-edge-style");
    const autoSubscriptInput = document.getElementById("setting-auto-subscript");
    const nodeColorInput = document.getElementById("setting-node-color");
    const forestLayoutSelect = document.getElementById("setting-forest-layout");
    const colorHint = document.getElementById("settings-color-hint");
    ...
    function focusSelectedNode() {
        ...
        container
            .querySelector(`[data-node-id="${target.id}"]`)
            ?.focus();
    }
    ...
    function fitToView() {
        ...
        const rect = treePane.getBoundingClientRect();
        const pad = 16;
        const z = clampZoom(Math.min((rect.width - pad) / baseW, (rect.height - pad) / baseH, 1));
        zoom = z;
        applyZoom();
    ...
    treePane.addEventListener("wheel", (e) => {
    ...
    const endPan = () => {
        if (panMoved)
            suppressClick = true; // don't let the pan end as a deselect
        panning = false;
    ...
    function handleLinkClick(node) {
        ...
        const ends = existing && existing.target === node ? nextArrowEnds(existing.ends) : nextArrowEnds(null);
        if (ends) {
            setArrow(linkSource, node, ends);
            flashStatus(ends.to && ends.from
    ...
    function refreshHighlight() {
        ...
        const active = document.activeElement === textInput &&
    ...
    textInput.addEventListener("keydown", (e) => {
        ...
        const val = el.value;
        const applyEdit = (text, selStart, selEnd = selStart) => {
            el.value = text;
        ...
        if (e.key === "[") {
            ...
            if (start === end)
                applyEdit(text, start + 1);
            else
                applyEdit(text, start + 1, end + 1);
    ...
    function startInlineEdit(node) {
        cancelInlineEdit();
        const group = container.querySelector(`[data-node-id="${node.id}"]`);
        if (!group)
            return;
        ...
        const finish = (commit) => {
            ...
            input.remove();
            if (inlineEditor === input)
                inlineEditor = null;
            if (inlineEditFinish === finish)
                inlineEditFinish = null;
            let changed = false;
            if (commit) {
                ...
                if (node.isWord)
                    node.triangle = derivedTriangle(base);
    ...
    const actions = {
        child() {
            ...
            if (sel)
                insertAndEdit(addChildAt(sel, sel.children.length));
        },
        "child-start"() {
            const sel = tree.selectedNode;
            if (sel)
                insertAndEdit(addChildAt(sel, 0));
        },
        text() {
            const sel = tree.selectedNode;
            if (sel)
                insertAndEdit(addChildAt(sel, sel.children.length, "", true));
        },
        "sib-before"() {
            const sel = tree.selectedNode;
            if (sel)
                insertAndEdit(addSiblingBefore(sel));
        },
        "sib-after"() {
            const sel = tree.selectedNode;
            if (sel)
                insertAndEdit(addSiblingAfter(sel));
        },
        copy() {
            if (tree.selectedNode) {
        ...
        rename() {
            if (tree.selectedNode)
                startInlineEdit(tree.selectedNode);
        },
        triangle() {
            const sel = tree.selectedNode;
            if (sel && sel.isWord) {
     ...
     * drawn with.
     */
    function bindNumberSetting(input, limit, apply) {
    ...
    edgeStyleSelect.addEventListener("change", () => {
        settings.edge.style = edgeStyleSelect.value;
        savePrefs();
        renderTree();
    });
    ...
    function restoreFromHistory(state) {
        ...
        if (!path)
            selectNode(null); // nothing was selected — keep it that way
        persistActive(state);
     ...
     * can't show.
     */
    function flashStatus(msg, action) {
    ...
    container.addEventListener("pointerdown", (e) => {
        if (!dragMode || !e.isPrimary || e.button !== 0)
            return;
    ...
    window.addEventListener("keydown", (e) => {
        ...
        if (typing)
            return; // don't hijack while editing text
        ...
        if (!tree.selectedNode) {
            ...
            const active = document.activeElement;
            const g = active instanceof Element && container.contains(active)
    ...
    // ---- export dialog --------------------------------------------------
    const exportFormatInputs = () => Array.from(exportModal.querySelectorAll('input[name="export-format"]'));
    const exportScopeInputs = () => Array.from(exportModal.querySelectorAll('input[name="export-scope"]'));
    const exportScaleSel = () => document.getElementById("export-scale");
    const exportTransparentBox = () => document.getElementById("export-transparent");
    const exportCombineBox = () => document.getElementById("export-combine");
    const exportCustomInput = () => document.getElementById("export-custom");
    ...
    const exportCopyImageBtn = () => document.getElementById("export-copy-image");
    const exportCopyCodeBtn = () => document.getElementById("export-copy-code");
    const exportCopyNoteEl = () => document.getElementById("export-copy-note");
    function currentExportFormat() {
        const picked = exportFormatInputs().find((i) => i.checked);
        return picked?.value ?? "png";
    }
     ...
     */
    function selectedTabIndices() {
        const scope = currentExportScope();
        if (scope === "all")
            return workspace.tabs.map((_, i) => i);
        if (scope === "custom")
     ...
     */
    function syncExportCopyButtons(format, count) {
    ...
    function syncExportUI() {
        ...
        const scope = currentExportScope();
        for (const row of exportModal.querySelectorAll("[data-format]")) {
        ...
        summary.textContent =
            count === 0
                ? "No tabs selected."
                : `${count} tab${count === 1 ? "" : "s"} → ${fileCount} ${ext.toUpperCase()} file${fileCount === 1 ? "" : "s"}.`;
    ...
    function syncExportInputs() {
        ...
        for (const i of exportFormatInputs())
            i.checked = i.value === p.format;
        for (const i of exportScopeInputs())
            i.checked = i.value === p.scope;
    ...
    async function buildExportFiles(indices) {
        const format = currentExportFormat();
        ...
        for (let k = 0; k < indices.length; k++) {
            ...
            if (format === "png")
                files.push(await pngFile(t, filenames[k], scale, transparent));
            else if (format === "svg")
                files.push(svgFile(t, filenames[k]));
            else
                files.push(latexFile(t, filenames[k]));
        }
        return files;
    }
    async function runExport(mode) {
        ...
        try {
            ...
            flashStatus(files.length === 1 ? `Saved ${files[0].filename}` : `Saved ${files.length} files`);
            exportModal.classList.remove("active");
        }
        catch (err) {
            exportErrorEl().textContent =
                err?.message || "The export failed. Try another format.";
        }
    }
    ...
    window.addEventListener("hashchange", () => {
        ...
        if (!doc || parseAll(doc).trees.length === 0)
            return;
        flushActiveText();
    ...
    function ensureHistory(id, baseline) {
    ...
    function loadActiveTab() {
        ...
        const loaded = parsed.length > 0 && !error ? parsed : parseAll(tab.text).trees;
    ...
    function flushActiveText() {
        ...
        if (parsed.length > 0 && !error)
            persistActive(textInput.value);
        else
            persistDraft(textInput.value);
    }
    ...
    function closeTab(id) {
        ...
        if (closedTabs.length > CLOSED_LIMIT)
            closedTabs.shift();
        if (wasActive)
            loadActiveTab();
        else
    ...
    function reopenTab() {
        ...
        if (entry.history)
            histories.set(entry.tab.id, entry.history);
        loadActiveTab();
        saveWorkspace(workspace.toStored());
    ...
    function renderTabs() {
        ...
        for (const tab of workspace.tabs) {
            ...
            if (tab.draft !== undefined)
                name.title = "Unparsed text waiting in this tab";
            el.appendChild(name);
    ...
    function tabDropIndex(clientX) {
        const els = Array.from(tabbar?.querySelectorAll("[data-tab-id]") ?? []);
    ...
    tabbar?.addEventListener("pointerdown", (e) => {
        ...
        if (target.closest("button") || target.closest("input"))
            return;
        const tabEl = target.closest("[data-tab-id]");
        if (!tabEl || workspace.tabs.length < 2)
            return;
    ...
    tabbar?.addEventListener("pointermove", (e) => {
        ...
        if (!tabDragArmed) {
            ...
            if (Math.abs(e.clientX - tabDragStartX) > 8)
                endTabDrag();
            return;
        }
        if (!tabDragMoved) {
            if (Math.abs(e.clientX - tabDragStartX) < 5)
                return;
            tabDragMoved = true;
            tabbar.setPointerCapture(e.pointerId);
        }
        e.preventDefault();
        if (workspace.move(tabDragId, tabDropIndex(e.clientX)))
            renderTabs();
        tabbar
            ?.querySelector(`[data-tab-id="${tabDragId}"]`)
    ...
    function renderHelpKeys() {
        ...
        for (const f of FIXED_KEYS)
            rows.push({ keys: f.keys, label: f.label });
    ...
    shortcutList?.addEventListener("click", (e) => {
        ...
        if (btn)
            beginCapture(btn.getAttribute("data-rebind"));
    });
    ...
    const shared = fragment && parseAll(fragment).trees.length > 0 ? fragment : null;
    if (restored) {
        ...
        const seed = legacy && parseAll(legacy).trees.length > 0 ? legacy : shared ?? DEFAULT_DOC;
