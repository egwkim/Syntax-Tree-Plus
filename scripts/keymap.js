...
{
    const reserve = (label, ...keys) => {
        for (const k of keys)
            RESERVED_KEYS.set(k, label);
    };
    ...
    for (const mod of ["Ctrl", "Meta"]) {
        ...
        reserve("switching tabs", `${mod}+Tab`, `${mod}+Shift+Tab`, `${mod}+Alt+ArrowLeft`, `${mod}+Alt+ArrowRight`, ...Array.from({ length: 9 }, (_, i) => `${mod}+${i + 1}`));
    }
}
/** What a structural key is already used for, or null if it's free to bind. */
export function reservedKey(canonical) {
...
export function applyOverrides(overrides) {
    ...
    for (const [id, key] of Object.entries(overrides)) {
        if (byId.has(id) && typeof key === "string" && key)
            bindings[id] = key;
    }
}
...
export function overrides() {
    const out = {};
    for (const c of COMMANDS)
        if (bindings[c.id] !== c.defaultKey)
            out[c.id] = bindings[c.id];
    return out;
}
export function bindingFor(id) {
    return bindings[id] ?? "";
}
export function commandDef(id) {
    return byId.get(id);
}
/** Which command owns a canonical key (primary or fixed alias), or null. */
export function commandForKey(canonical) {
    for (const c of COMMANDS) {
        if (bindings[c.id] === canonical)
            return c;
        if (c.extraKeys && c.extraKeys.includes(canonical))
            return c;
    }
    return null;
}
 ...
 * success.
 */
export function rebind(id, canonical) {
    if (!byId.has(id))
        return null;
    const reserved = reservedKey(canonical);
    if (reserved)
        return { id: "", label: reserved };
    for (const c of COMMANDS) {
        if (c.id === id)
            continue;
...
export function bindingToDefault(id) {
    const def = byId.get(id);
    if (def)
        bindings[id] = def.defaultKey;
}
/** Canonical binding string for a keydown event (see module doc). */
export function canonicalFromEvent(e) {
    ...
    const shiftNamesTheKey = e.shiftKey && !(key.length === 1 && key.toLowerCase() === key.toUpperCase());
    const parts = [];
    if (e.ctrlKey)
        parts.push("Ctrl");
    if (e.metaKey)
