import { settings, SETTING_LIMITS, clampSetting } from "./settings.js";
...
export function saveKeymap(overrides) {
    try {
 ...
 */
export function saveToolbarCat(id) {
    try {
...
export function loadToolbarCat() {
    try {
...
export function loadPrefs() {
    ...
    if (typeof p.horizontalSpacing === "number" && Number.isFinite(p.horizontalSpacing))
        settings.node.horizontalSpacing = clampSetting(p.horizontalSpacing, SETTING_LIMITS.horizontalSpacing);
    if (typeof p.verticalSpacing === "number" && Number.isFinite(p.verticalSpacing))
        settings.node.verticalSpacing = clampSetting(p.verticalSpacing, SETTING_LIMITS.verticalSpacing);
