 ...
 */
const COMPACT_QUERY =
  "(max-width: 760px), (max-height: 520px) and (pointer: coarse)";

export function setupCompactToolbar() {
  const toolbar = document.getElementById("toolbar");
  const cats = document.getElementById("toolbar-cats");
  if (!toolbar || !cats) return;

  const groups = Array.from(
    toolbar.querySelectorAll<HTMLElement>(".group[data-cat]")
  );
  if (groups.length === 0) return;

  const idOf = (g: HTMLElement) => g.dataset.cat!;
  const labelOf = (g: HTMLElement) => g.dataset.catLabel || idOf(g);
  ...
  let active: string | null = stored === null ? idOf(groups[0]) : stored || null;
  if (active !== null && !groups.some((g) => idOf(g) === active)) {
    active = idOf(groups[0]);
  }

  for (const g of groups) {
    if (!g.id) g.id = "tb-group-" + idOf(g);
  ...
  function apply() {
    for (const g of groups) g.classList.toggle("cat-active", idOf(g) === active);
    for (const chip of cats!.querySelectorAll<HTMLElement>(".cat")) {
      chip.setAttribute("aria-expanded", String(chip.dataset.cat === active));
    }
  }

  cats.addEventListener("click", (e) => {
    const chip = (e.target as Element).closest<HTMLElement>(".cat");
    if (!chip) return;
  ...
  else (mq as MediaQueryList).addListener(sync); // older WebKit
  sync();
  apply();
}
