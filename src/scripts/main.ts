...
import { renderTreeToSVG } from './render.js';
import { editSelectedLabel, deleteSelectedNode, cloneNodeSubtree } from './edit.js';
...
window.addEventListener("keydown", function (e: KeyboardEvent) {
  ...
  if (e.key === "ArrowUp") {
    ...
    editSelectedLabel(tree, render);
  } else if (e.key === "Delete" || e.key === "Backspace" || e.key === "d") {
    deleteSelectedNode(tree, render);
  } else if (e.key === "x") {
    if (node && node.parent) {
      clipboardNode = node;
